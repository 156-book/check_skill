...
def main() -> None:
    ...
    fixture_lines = fixture_bytes.decode("utf-8").splitlines()
    ...
    actual_hash = hashlib.sha256(fixture_bytes).hexdigest().upper()
    if actual_hash != EXPECTED_FIXTURE_SHA256:
        raise AssertionError(
            "测试文档已被修改："
            f"期望 {EXPECTED_FIXTURE_SHA256}，实际 {actual_hash}"
        )
    ...
    print(f"PASS: 测试文档 SHA-256 = {actual_hash}")
